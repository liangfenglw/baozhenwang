...
//	console.log("data3:", JSON.stringify(data3, null, "\t"));		//格式化输出json
	...
	$("#lanmu_leixing").change(function(){
		fenlei = $(this).val();
		//获取拼接域名地址：Admin/goods/Businessort
        ...
        $.ajax({
            ...
            success: function (data) {
              ...
              if(num >=1) {
                  ...
                  $("#shangpin_fenlei").append("<option>一级栏目</option>")
                  for (var i = 0; i < num; i++) {
                      var heml = "<option value=" + data.data[i].id + ">" + data.data[i].name + "</option>";
                      $("#shangpin_fenlei").append(heml);
                  }
              }
            }
        });
...
//编辑器
	...
	// 上传图片
	editor.config.uploadImgUrl = uploadImgUrl_;
	// 配置自定义参数
	...
	editor3.config.menus = $.map(wangEditor.config.menus, function(item, key) {
		if (item === 'location') {	return null;	}
		return item;
	});
	editor4.config.menus = $.map(wangEditor.config.menus, function(item, key) {
		if (item === 'location') {	return null;	}
		return item;
	});
	editor5.config.menus = $.map(wangEditor.config.menus, function(item, key) {
		if (item === 'location') {	return null;	}
		return item;
	});
	editor7.config.menus = $.map(wangEditor.config.menus, function(item, key) {
		if (item === 'location') {	return null;	}
		return item;
	});
